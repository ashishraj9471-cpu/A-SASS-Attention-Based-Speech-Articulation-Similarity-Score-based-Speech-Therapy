...
# =====================================================
# WHISPER FFMPEG PATCH (Fixes FileNotFoundError on Windows)
# =====================================================
...
# Direct override for Whisper's internal audio loader to use imageio-ffmpeg
def patched_load_audio(file: str, sr: int = whisper.audio.SAMPLE_RATE):
  ...
  try:
    out = whisper.audio.run(cmd, capture_output=True, check=True).stdout
  except whisper.audio.CalledProcessError as e:
    raise RuntimeError(f"Failed to load audio: {e.stderr.decode()}") from e

  return (
      whisper.audio.np.frombuffer(out, np.int16).flatten().astype(np.float32)
      / 32768.0
  )
...
# Register pillow_heif for HEIC format support
...
st.set_page_config(
    page_title="A-SASS Speech Therapy", page_icon="🧠", layout="wide"
)
...
def record_video(output_path: str, duration: float):
  ...
  writer = cv2.VideoWriter(
      output_path, cv2.VideoWriter_fourcc(*"mp4v"), 20, (640, 480)
  )
...
def word_diff_html(ref_text: str, hyp_text: str):
  ...
  M = (
      "background:#d4edda;color:#155724;border-radius:4px;padding:2px"
      " 5px;margin:2px;display:inline-block;"
  )
  RM = (
      "background:#f8d7da;color:#721c24;border-radius:4px;padding:2px"
      " 5px;margin:2px;display:inline-block;text-decoration:line-through;"
  )
  HM = (
      "background:#fff3cd;color:#856404;border-radius:4px;padding:2px"
      " 5px;margin:2px;display:inline-block;"
  )
  SR = (
      "background:#f8d7da;color:#721c24;border-radius:4px;padding:2px"
  ...
  for op, i1, i2, j1, j2 in matcher.get_opcodes():
    if op == "equal":
      for w in ref_words[i1:i2]:
        rp.append(f'<span style="{M}">{w}</span>')
      for w in hyp_words[j1:j2]:
        hp.append(f'<span style="{M}">{w}</span>')
    elif op == "delete":
      for w in ref_words[i1:i2]:
        rp.append(f'<span style="{RM}">{w}</span>')
    elif op == "insert":
      for w in hyp_words[j1:j2]:
        hp.append(f'<span style="{HM}">{w}</span>')
    elif op == "replace":
      for w in ref_words[i1:i2]:
        rp.append(f'<span style="{SR}">{w}</span>')
      for w in hyp_words[j1:j2]:
        hp.append(f'<span style="{SH}">{w}</span>')
...
if language == "Hindi" and selected_model in ["small"]:
  st.warning(
      "⚠️ 'small' model may romanize Hindi. Medium or large-v2 is strongly"
      " recommended."
  )
...
if st.session_state["therapist_saved"]:
  ...
  if input_mode == "Upload Video":
    ...
    st.info(
        f"Recording duration will match therapist video: {ref_duration:.2f} sec"
    )
...
if st.session_state["therapist_saved"] and st.session_state["patient_saved"]:
  ...
  if st.button(
      "🔬 Run A-SASS Analysis", use_container_width=True, key="sass_run_btn"
  ):
    ...
    with st.spinner("Running Analysis..."):
      ...
      from dashboard import articulation_radar, attention_chart, show_dashboard
      from face_features import compare_features, extract_features
      ...
      ref_result = transcribe(
          whisper_model, therapist_audio, language, TEMP_DIR
      )
      ...
      phoneme_results = analyze_phonemes(
          therapist_text, patient_text, language
      )
  ...
  if st.session_state["results"] is not None:
    ...
    if R.get("ref_romanized"):
      st.warning(
          "⚠️ Therapist transcription may be romanized. Try large-v2 model."
      )
    if R.get("hyp_romanized"):
      st.warning(
          "⚠️ Patient transcription may be romanized. Try large-v2 model."
      )
    ...
    if score >= 90:
      st.success(
          "✅ Excellent articulation. Patient closely matches the reference."
      )
    elif score >= 80:
      st.success("✅ Good performance. Minor improvements needed.")
    elif score >= 70:
      st.warning(
          "⚠️ Moderate articulation quality. Continued practice recommended."
      )
    elif score >= 60:
      st.warning("⚠️ Needs additional practice. Focus on highlighted words.")
    else:
      st.error(
          "❌ Significant articulation issues detected. Intensive therapy"
          " advised."
      )
